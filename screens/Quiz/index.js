...
export default function Quiz({ navigation }) {
  ...
  async function startQuiz() {
    if (!selectedTheme || questionCount <= 0) {
      Alert.alert(
        "Error",
        "Please select a topic and set the number of questions."
      );
      return;
    }
    ...
    if (questionCount > questionsCountByTheme[selectedTheme]) {
      Alert.alert(
        "Error",
        `The selected topic has only ${questionsCountByTheme[selectedTheme]} questions.`
      );
      return;
    }
            ...
            {themes.map((theme) => (
              ...
              >
                <Text style={styles.themeText}>
                  {theme.name} - {questionsCountByTheme[theme.id]} question(s)
                </Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
